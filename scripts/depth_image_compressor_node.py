...
def _cv_bridge_callback(img, bridge, pub_compresse):
    ...
    msg.format = "jpeg"
    compressed_img = np.array(cv2.imencode('.jpg', cv_image)[1])
    msg.data = compressed_img.tostring()
    pub_compresse.publish(msg)

rospy.init_node('cv_bridg', anonymous=True)
bridge = CvBridge()
pub_compresse = rospy.Publisher("depth/compressed", CompressedImage, queue_size=2)
rospy.Subscriber("camera/depth/image_rect_raw", Image, lambda img: _cv_bridge_callback(img, bridge, pub_compresse), queue_size=2)
