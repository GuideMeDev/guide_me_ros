...
from Modules.plane_fit_v2 import plane_fit_v2
from Modules.scan_match import scan_match
from Modules.SLAM import SLAM
from Modules.plane_fit import plane_fit
from utils import *
import cProfile, pstats, io
from multiprocessing import Pool    # for all other modules - running multiproccess to lower running time.
import threading    # for plane fit + read_data

def run_algo(dxinternal,dyinternal,I,pcloud,pRGB1,yaw,XYZ,roll,pitch):
    tx=np.zeros((len(I),2))
    S = []
    tr = []
    tc = []
    ...
    for i in range(len(I)-2):
        # Module 1 - plane_fit: In order to run it real-time - change plane_fit's input and code for discrete data (one frame)
        # Scan Match module
        # Runtime performance profiler    
        yaw_t,tx_curr,minter_plus,minter_minus = scan_match(pcloud[i],pcloud[i+1],pRGB1[i],pRGB1[i+1],yaw[i],yaw[i+1],dxIMU[i],dyIMU[i],tx_prev,0)
        yawt.append(yaw_t)
        tx[i,:] = tx_curr
        # a=I[i]
...
if __name__ == "__main__":
    ...
    # Downsampling pcloud to half the resolution - every second element
    # for i in range(len(pcloud)):
    #     pcloud[i] = pcloud[i][::2]
    pr = cProfile.Profile()
    pr.enable()
    pcloud1 = plane_fit_v2(I,XYZ,roll,pitch)
    run_algo(dt[0],dt[1],dt[2],pcloud1,dt[4],dt[5],XYZ,roll,pitch)
    pr.disable()
    s = io.StringIO()
