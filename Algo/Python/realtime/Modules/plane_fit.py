...
def plane_fit(I, XYZ, roll, pitch,prev_group,h1_que,h1_prev = INIT_H1):
    sc2=0.5
    no_plane=0
    #h1 = h1_prev - 2*sc_acc
    h1 = h1_prev
    ...
    if len(f):
        f=f[np.random.randint(len(f),size = 200)]

    pcloud = x3
    return pcloud,h1,eul,fr,prev_group
...
if __name__ == '__main__':
    ...
    for frame in frames:
        if h0 == 0:
            ...
            plane_fit(frame[0],frame[1],frame[3][0][0],frame[3][0][1],h0)
