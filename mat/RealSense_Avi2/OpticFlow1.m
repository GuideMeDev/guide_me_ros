...
%%
% figure('units','normalized','outerposition',[0 0 1 1])
x=[];sc=1;fc=1.93;sopix=1*3e-3/sc;%a=rgb2gray(I{i});imshow(a)%a=fliplr(rgb2gray(I)')';
kr=30;kc=180;yaw1=[];dx1=[];M=[];
% 
for i=1:length(I)%66%n1-1
a=rgb2gray(I{i});px0=size(a,2)/2;py0=size(a,1)/2;%imshow(a)  
% [SWpixl,repvfl,ll,m]=Image_SW_Projection_5(imadjust(a,[0.5,0.8],[0,1])-1,[eul2(i,1:4),5],15,f,sopix,px0,py0,[],1e3*x3{i},0);
if length(pcloud{i})>1e4
[SWpixl,repvfl,ll,m]=Image_SW_Projection_5(imadjust(a,[0.3,0.7],[0,1])-1,[eul(i,1:3),h1(i),5],15,fc,sopix,px0,py0,[],1e3*pcloud{i},0);
m1=m(:,:,1);f=m1==0;m1(f)=mean2(m1(m1>0));c=conv2(m1,ones(4,4)/16,'same');m1=c>0.8;%imagesc(c>0.8)
% m1a=m1*0;m1a(1+kr:end-kr,1+kc:end-10,:)=m1(1+kr:end-kr,1+kc:end-10,:);
% m=m(1+kr:end-kr,1+kc:end-10,:);
% m=(m(:,:,1).*(m(:,:,2)>0));
% t=mean2(m(m>0.2));m(m<=0.2)=t;m=m-t;
% m(m<0)=-10;m(m>0)=10;%imagesc(m);surf(m)
% t=m*0;t(12:end-11,12:end-11)=m(12:end-11,12:end-11);
t=double(m1);M{i}=t;st=size(t);%imshow(M{i})
% m=(m(:,:,1).*(m(:,:,2)>0));M{i}=m;st=size(m);m=m-mean2(m);M{i}=m;%imshow(M{i})
if i>1%&&sum(sum(m>0))>2e3&&sum(sum(M{i-1}>0))>2e3
%     pause
m1=M{i-1};m2=M{i};m1(:,270:end)=0;
[py,px]=find(m1~=0);f_ind=find(m1~=0);py=py-st(1)/2;%[py2,px2]=find(m2>0);py2=py2-st(1)/2;%b1=B{i-1};px=b1(:,1);py=b1(:,2);
% s=[];yaw1=yawt(1,i)+[-0.008,0,0.008];
s=[];yaw1=[-5:0.5:5]*pi/180;
for j1=1:length(yaw1)
tetaz=yaw1(j1);Rz=[cos(tetaz),-sin(tetaz);sin(tetaz),cos(tetaz)];t1=round((Rz*[px';py'])');px1=t1(:,1);py1=t1(:,2)+st(1)/2;
m1a=zeros(st(1),st(2));m1a((px1-1).*size(m1,1)+py1)=m1(f_ind);%imshow(mpc1)
s(j1)=sum(sum((m2-m1a).^2));
end
fs=find(s==min(s));fs=fs(1);tetaz=yaw1(fs);eulcam(i,1)=tetaz;Rz=[cos(tetaz),-sin(tetaz);sin(tetaz),cos(tetaz)];t1=round((Rz*[px';py'])');px1=t1(:,1);py1=t1(:,2)+st(1)/2;
m1a=zeros(st(1),st(2));m1a((px1-1).*size(m1,1)+py1)=m1(f_ind);Ma{i-1}=m1a;
m=m2;m(:,:,3)=m1a;imshow(m)
% m1=Ma{i1-1};m2=M{i1};%m1=m1(1:2:end,1:2:end);m2=m2(1:2:end,1:2:end);
% k1=30;k2=30;
...
% m=m1;m(k+dx(1):end-k+dx(1),k+dx(2):end-k+dx(2),3)=m2(k:end-k,k:end-k);
% m=m2;m(k+dx(1):end-k+dx(1),k+dx(2):end-k+dx(2),3)=m1a();
m=m2;m(k:end-k,k:end-k,3)=m1a(k+dx(1):end-k+dx(1),k+dx(2):end-k+dx(2));m(st(1)/2-15:st(1)/2+15,1:100,2)=1;
subplot(2,2,2)
imagesc(tt(rangett(1)-k1:rangett(1)+k1,rangett(2)-k1:rangett(2)+k1))
subplot(2,2,1)
imshow(I{i})
subplot(2,2,[3,4])
imshow(imrotate(fliplr(m),-90))
pause(.1)
dx1(i,:)=dx;
